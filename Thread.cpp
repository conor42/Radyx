...
#ifdef _WIN32
	SetThreadPriority(HANDLE(thread.native_handle()), THREAD_PRIORITY_BELOW_NORMAL);
#endif
}
...
void Thread::SetWork(std::function<void(void*, int)> fn, void *argp_, int argi_)
{
	std::unique_lock<std::mutex> lock(mutex);
	work_fn = fn;
	argp = argp_;
